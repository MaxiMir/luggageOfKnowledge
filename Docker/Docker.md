...
docker ps --all # список всех контейнеров (по умолчанию показывает запущенные)
...
COPY . . # копируем оставшиеся элементы из корня (где лежит Dockerfile) в место образа app(тк указано в WORKDIR)
...
```shell
docker build . # создание локального image
docker run 46e6b865f6658 # запуск image (run для работы с images)
docker run -d -p 80:3000 --name logsapp # запуск image -p локальный порт:порт в докере(EXPOSE) / -d без запуска консоли
docker start 46e6b865f6658 # для повторного запуска (start для работы с контейнерами)
