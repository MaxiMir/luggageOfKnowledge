...
| `object`    | `object`           | Массивы, объекты, функции, null                           |
| `function`  | <->                | Особый случай объекта                                     |
| `class`     | `function`         | Особый случай объекта                                     |
...
### 🔢 Примеры уровней на практике

| Протокол / технология     | Уровень OSI       | Что делает                                |
|---------------------------|-------------------|-------------------------------------------|
| **HTTP / HTTPS**          | 7 (Прикладной)    | Отправка HTML, JSON, файлов               |
| **TLS / SSL**             | 6 (Представления) | Шифрование / дешифрование данных          |
| **TCP / UDP**             | 4 (Транспортный)  | Надёжная доставка (TCP) или быстрая (UDP) |
| **IP / ICMP**             | 3 (Сетевой)       | Доставка пакетов от точки A к точке B     |
| **Ethernet / Wi-Fi**      | 2 (Канальный)     | Связь на уровне сетевой карты             |
| **Кабели, радио, лазеры** | 1 (Физический)    | Передача битов 1/0 как сигналы            |
...
| ⚡ Современные версии          | IP v4 / v6                     | TCP                                         | HTTP/1.1, HTTP/2, HTTP/3 (с QUIC вместо TCP) | HTTP/2, HTTP/3 + TLS                                      |
...
| Скорость доступа | Быстрое добавление и удаление (O(1)) |
...
|-----------------|-----------|----------|----------|--------------------------------|
| **Stack**       | O(n)      | O(1)     | O(1)     | Рекурсия, call stack           |
| **Queue**       | O(n)      | O(1)     | O(n)     | События, BFS                   |
