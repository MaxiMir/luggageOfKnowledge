...
> **LexicalEnvironment** - скрытый внутренний объект у выполняемых функций / блока кода / скрипта.
...
| ⏳ Время жизни                | Указывается явно (Max-Age и Exipers) или до окончания сессии | Без срока действия (пока не удалят) | До закрытия вкладки или окна    |
...
- Определяет, что IP-адрес находится вне текущей подсети — использует шлюз (обычно Wi-Fi роутер).
...
> MAC-адрес (Media Access Control address) — это уникальный идентификатор сетевого интерфейса (например, Wi-Fi- или Ethernet-адаптера) на канальном уровне (L2 OSI). Используется при передаче данных в локальной сети.
