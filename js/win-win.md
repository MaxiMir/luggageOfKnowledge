...
### 🧩 1. DNS-запрос — узнать IP сайта

Браузер или ОС запрашивает:
...
* свой внутренний кэш
* кэш операционной системы (DNS-кэш)

Если нет, происходит DNS-запрос (UDP-пакет):

* либо к DNS-серверу от провайдера
* либо к 8.8.8.8 (Google DNS), если так настроено

➡️ Ответ: 142.250.190.14 (например)

### 📦 2. Формируется HTTP-запрос (на https://google.com)

Браузер:

* формирует HTTPS-запрос
* упаковывает его в TCP-пакет (потому что HTTPS работает поверх TCP)
* добавляет IP-адрес назначения

➡️ Теперь нужно отправить этот пакет

### 📡 3. Отправляется запрос по Wi‑Fi

Ethernet здесь нет, но работает аналогичный протокол — Wi‑Fi (IEEE 802.11).

Он тоже:

* использует MAC-адреса
* локален (в пределах твоей сети)
* и нужен для передачи пакета до роутера

**Что делает ноутбук:**

* Понимает, что IP 142.250.190.14 не из локальной сети
* Отправляет IP-пакет роутеру, но...
* Wi‑Fi не работает с IP — он требует MAC

➡️ Выполняется ARP-запрос:
> "Кто такой 192.168.0.1 (роутер)?"

Роутер отвечает:
> "Вот мой MAC: AA:BB:CC:DD:EE:FF"

Теперь ноутбук:

* Упаковывает IP-пакет в Wi‑Fi-кадр
* Указывает MAC роутера как получателя
* И передаёт этот кадр по радиоканалу (через антенну)

### 🌐 4. Роутер получает кадр

* Извлекает IP-пакет
* Понимает, что IP-адрес назначения — внешний
* Делает NAT (подменяет внутренний IP на внешний)
* Отправляет пакет через провайдера в интернет

Теперь пакет уходит дальше через маршрутизаторы провайдеров…

> NAT (Network Address Translation) — это механизм, который позволяет нескольким устройствам в локальной сети (например,
> дома), выходить в интернет через один внешний IP-адрес (IP провайдера)

Например:

* ноутбук → IP `192.168.0.5`
* телефон → IP `192.168.0.10`
* роутер → IP `192.168.0.1` (локальный) и внешний IP от провайдера, например `92.143.25.11`

Все устройства "прячутся" за роутером, и когда ты заходишь на сайт, сервер видит только один внешний IP - `92.143.25.11`

🔁 Что делает NAT?

📤 Когда ноутбук отправляет запрос на google.com:

* У него IP: `192.168.0.5`
* Он отправляет пакет на роутер
* Роутер:
    * 🧠 Запоминает: «исходящий порт `50123` принадлежит `192.168.0.5:50123`»
    * 🔁 Меняет IP-адрес отправителя на свой внешний `92.143.25.11`
    * 📤 Отправляет пакет в интернет

📥 Когда приходит ответ от Google:

* Он приходит на `92.143.25.11:50123`
* Роутер:
    * 🧠 В таблице находит: «это для 192.168.0.5:50123»
    * 🔁 Меняет адрес назначения обратно
    * 📬 Отправляет ноутбуку
    * Он приходит на `92.143.25.11:50123`

### 🧭 5. Межсетевой маршрут (через интернет)

Пакет проходит:

* провайдера (ISP)
* региональный магистральный маршрутизатор
* ядро интернета (BGP маршрутизация)
* доходит до ближайшего дата-центра Google

➡️ Там он обрабатывается сервером Google, и тебе возвращается HTML-ответ.

### 🛰 6. Обратный путь — точно так же

* Ответ Google идёт обратно по IP
* Доходит до твоего роутера
* Роутер по NAT знает, кому вернуть — твоему ноутбуку
* Упаковывает в Wi‑Fi-кадр и отправляет тебе

Твой ноутбук принимает кадр по радио и:

* Распаковывает IP → TCP → HTTPS → HTML
* Показывает страницу

---

### 🍔 Сетевой «бутерброд» (инкапсуляция)

```
[Ethernet-кадр [    # Добавляется MAC-адреса и контрольная сумма
   IP-пакет [       # Добавляется IP-заголовок (IP-адреса отправителя и получателя)
     TCP-сегмент [  # Добавляется TCP-заголовок (порты, флаг SYN, и т.п.)
       HTTP-запрос  # GET /
     ]
   ]
]]
...
ℹ️ Порт в TCP — это числовой идентификатор (от `0` до `65535`), который помогает ОС понять Какому приложению передать
данные, пришедшие по сети.

ℹ️ SYN в TCP — это сигнал: «Я хочу установить TCP-соединение»
...
| Поле            | Значение                                |
|-----------------|-----------------------------------------|
| **Source Port** | `50231` — случайный порт браузера       |
| **Dest Port**   | `443` — порт HTTPS сервера Google       |
| **Flags**       | `SYN` — начало соединения               |
| **Seq Number**  | `0` (начальный порядковый номер пакета) |

👉 Это первый шаг TCP-handshake. Только SYN. Нет данных пока.
...
Frame 2: 66 bytes
  TCP
Source Port: 443
Destination Port: 50231
Flags: 0x012 (SYN, ACK)
Sequence Number: 0
Ack Number: 1
```

> ACK — это сокращение от acknowledgment (англ. "подтверждение").
>
> 📨 «Я получил твои данные, всё в порядке, можешь отправлять дальше».
>
> 🔄 Повторно отправить, если подтверждение не пришло (timeout)

| Поле            | Значение                                 |
|-----------------|------------------------------------------|
| **Source Port** | `443` — сервер Google                    |
| **Dest Port**   | `50231` — порт клиента                   |
| **Flags**       | `SYN, ACK` — установка + подтверждение   |
| **Ack Number**  | `1` — подтверждает получение первого SYN |
...
🎉 Всё — соединение установлено! Теперь можно отправлять HTTP-запрос внутри TCP.
...
> MAC-адрес (Media Access Control address) — это уникальный физический адрес сетевого интерфейса устройства (например,
> Wi-Fi или Ethernet-карты).
...
|-------------------|--------------------------------------------------------------|
| 🔢 Формат         | 48 бит (обычно записывается в виде 6 байтов в hex)           |
| 🆔 Уникальность   | Почти уникален — выдается производителем сетевого устройства |
| 🌐 Уровень OSI    | **Канальный уровень (Layer 2)**                              |
| 🔄 Изменяемость   | Аппаратно задаётся, но можно подменить (spoof)               |
| 🧭 Назначение     | Идентификация устройств в **локальной сети (LAN)**           |
| 🧩 Используется в | Ethernet, Wi-Fi, Bluetooth и других L2-протоколах            |

### 🧬 Локальный VS публичный MAC

| Характеристика           | Публичный MAC                  | Локальный MAC                     |
|--------------------------|--------------------------------|-----------------------------------|
| 📍 Назначение            | Назначен производителем        | Назначен вручную или ОС           |
| 🧬 Уникальность          | Глобально уникален             | Может повторяться                 |
| ⚙️ Как определяется      | Первый байт: 2-й бит = `0`     | Первый байт: 2-й бит = `1`        |
| 👀 Где используется      | Устройства с заводским адресом | Для приватности, временные сессии |
| 🛡 Приватность           | Нет                            | Да — может скрывать личность      |
| 📱 Примеры использования | Сетевая карта ноутбука         | Случайный MAC на смартфоне        |

ℹ️ Смартфоны (iOS/Android) используют рандомизированный MAC при сканировании Wi-Fi сетей, чтобы:
...
> Маршрутизатор — это устройство или программа, которая направляет сетевые пакеты между сетями, выбирая лучший маршрут
> для их доставки.

### 📌 Где они используются?

* В домашней сети - Wi-Fi роутер
* В корпоративных сетях — маршрутизаторы соединяют отделы, филиалы, дата-центры
* В интернете — огромные магистральные роутеры провайдеров соединяют миллионы сетей

Между странами и провайдерами работают магистральные маршрутизаторы.
Они обмениваются маршрутами по протоколу BGP.
Именно они определяют, по каким путям идут пакеты по планете.

### 🧠 Что делает маршрутизатор?

1. Получает IP-пакет
2. Читает адрес назначения (IP) в заголовке
3. Смотрит таблицу маршрутов (routing table)
4. Выбирает наилучший следующий хоп (next hop)
5. Пересылает пакет на нужный интерфейс (провод, Wi-Fi, провайдер и т.д.)
