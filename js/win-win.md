...
- Параллельно работает **спекулятивный Preload Scanner**:
    ...
    - инициирует **раннюю загрузку**, ещё до обработки этих тегов основным парсером.
...
- Браузер объединяет **DOM** и **CSSOM**, чтобы построить **Render Tree** — структуру, отражающую визуальные элементы страницы.
...
- Отдельные логические слои (stacking contexts, `transform`, `opacity`, `position`, `z-index`) выделяются в **compositing layers**.
...
- Главный поток браузера работает по **циклу событий (Event Loop)**:
    ...
    2. Выполнение `requestAnimationFrame` и других animation callbacks,
    3. Layout и style updates при необходимости,
    4. Paint и Compositing.
