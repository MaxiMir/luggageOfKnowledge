...
✅ Результат: TCP-соединение установлено, данные можно передавать (например, HTTP-запрос).
...
- ОС обрабатывает протоколы сетевого стека:
    * принимает Wi‑Fi кадр → извлекает IP-пакет → передаёт TCP-сегмент приложению.
...
- Скрипты с `defer` загружаются параллельно и выполняются **после построения DOM** (затем
...
- Тип подключения влияет на выполнение:
    ...
    - `defer` — загружается параллельно, выполняется после построения DOM, в порядке подключения,
...
> Reflow вызывается изменением DOM, стилей или чтением layout-свойств (offsetWidth, offsetHeight, scrollWidth,
...
- Изменения DOM или стилей, затрагивающие геометрию, могут вызывать повторные перерасчёты (**reflow**, **layout
  invalidations**).
- Частые или неэффективные reflow могут существенно влиять на производительность (например, из-за «layout thrashing»).
...
- Для каждой видимой части Render Tree браузер формирует **Display List** — последовательность инструкций рисования:
    - примитивы: границы, фон, текст, изображения, тени, SVG и др.
...
- Отдельные логические области страницы (например, stacking contexts, элементы
  с `transform`, `opacity`, `position`, `z-index`, `will-change`) выделяются в **compositing layers**.
- Каждый слой рендерится на отдельную GPU-поверхность (texture).
- Браузер формирует **командный буфер** с инструкциями для GPU.
- GPU **собирает (blends)** слои в нужном порядке с учётом прозрачности и наложения.
- Изменение свойств, таких как `transform` или `opacity`, требует только обновления композиции, без повторного Paint.
