...
- Скрипты с `defer` загружаются параллельно и выполняются **после построения DOM** (потом
...
- `<link rel="stylesheet">` без указания `media`, либо с `media`, применимым к текущему окружению например, `screen`,
  блокирует рендеринг до загрузки и построения CSSOM.
...
- Для каждого визуального элемента рассчитываются:
