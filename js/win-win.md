...
- Парсер поочерёдно читает поток символов и выполняет:
    1. **Токенизацию** — преобразование текста в последовательность токенов (теги, текст, комментарии и пр.).
    2. **Построение DOM-дерева** — из токенов создаётся структура **Document Object Model**.
...
- Происходит поэтапная обработка:
    1. **Парсинг** → преобразование в абстрактное синтаксическое дерево (AST),
    2. Компиляция в **байткод или машинный код** с помощью JIT (Just-In-Time),
    3. **Исполнение** — запуск кода на виртуальной машине.
