...
**Шаг 1**. Клиент → Сервер посылает `SYN`

> SYN в TCP — это сигнал: «Я хочу установить TCP-соединение»
...
> Сервер подтверждает получение SYN и сообщает свой SYN
...
Flags: 0x012 (SYN, ACK)        # синхронизация + подтверждение
Sequence Number: ⟨server_ISN⟩  # случайное начальное число сервера
Ack Number: ⟨client_ISN + 1⟩   # подтверждение первого SYN
```

---
✅ **Шаг 3**. Клиент подтверждает `ACK`

> Клиент подтверждает получение SYN сервера
...
### 🔒 TLS Handshake (HTTPS-шифрование)
...
📤 **Шаг 1**. Клиент → Сервер: `ClientHello` и

```
ClientHello
- supported TLS version   # список поддерживаемых версий TLS (прим. 1.2, 1.3)
- supported ciphers       # список поддерживаемых шифров
- client_random           # случайный набор байтов
```

---
📥 **Шаг 2**. Сервер отвечает: `ServerHello` и

``` 
ServerHello
- selected_version      # выбранная версия TLS
- selected_cipher       # выбранный шифр
- server_random         # случайный набор байтов
- (TLS 1.3) key_share   # параметры (ECDHE) сервера
...
* Клиент и сервер обмениваются ключевыми материалами (RSA-зашифрованный pre-master-секрет или параметры Диффи-Хеллмана),

* 🔑 по результату вычисляют общий мастер-ключ на основе ключевого материала и случайных наборов байтов.

> **Протокол Диффи-Хеллмана** позволяет двум и более сторонам получить общий секретный ключ, используя незащищенный от
> прослушивания канал связи

--- 
✅ **Шаг 5**. Подтверждение безопасности

* Клиент и сервер посылают сообщения "Finished", зашифрованные этим ключом
* После этого весь канал (TLS Records) шифруется симметричным ключом, и начинается защищённый обмен HTTP-сообщениями.

---

### ✏️ Формирование HTTPS запроса

Браузер:

- формирует запрос (например `GET / HTTP/2`).
- упаковывает в TCP-пакет
- добавляет IP-адрес назначения

---

### 📤 Передача по локальной сети (Wi-Fi)

- Определение, что целевой IP вне подсети — нужен шлюз.
- ARP-запрос для получения MAC-адреса роутера.
- Отправка кадра Wi‑Fi (IEEE 802.11) на MAC роутера.

```
[Wi-Fi-кадр [       # Добавляется MAC-адреса и контрольная сумма
...
- Роутер извлекает IP-пакет
- Роутер заменяет внутренний IP/порт на свой внешний IP с динамически выбранным внешним портом.
...
- Сервер возвращает зашифрованный HTTP-ответ (HTML, JS, CSS).
- Пакет следует обратным маршрутом.
- Роутер по таблице NAT восстанавливает внутренний адрес и порт.
- Кадр Wi-Fi доставляется ноутбуку.

--- 

### ✨Декодирование на клиенте

- Снятие инкапсуляции: Wi-Fi → IP → TCP → TLS → HTTP.
- Передача данных в процесс браузера.
