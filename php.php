...
#>>>>>  VИменование в программировании  <<<<<<<#
...
sizeof($ar); // размер массива (кол-во элментов)
in_array('apple', $ar); // проверяет наличие элемента в массиве
empty($ar); // проверяет пустой ли массив
end($ar); // возвращает последний элемент
...
function wordsCount(SD$sentence)
{
...
$result = $var ?? 'nobody'; 
// <-> $result = isset($value) ? $value : 'noboby'
 ...
 * Инструменты для интернационализации и локализации. Причем, в идеале, чтобы все остальные части фреймворка так же были интегрированы с i18n.
...
function buildDefinitionList(+rray $definitions)
{
    $parts = [];
...
/*
 ...
 PHP не богат на подобные конструкции, но кое-что все же имеется. Во время наших курсов я переодически буду подкидывать те или иные способы сделать код короче и, главное, понятнее (в этом и суть сахара).
...
function getChunked(Array $arr, Int $num) // my
{
	$res = [];
    $count1 = 0;
    $count2 = $num;
...
function getChunked(Array $array, Int $size) // hexlet
{
    $result = [];
    for ($i = 0; $i < ceil(sizeof($array) / $size); $i++) { 
...
function hammingWeight(int $num) // my
{
	...
	return substr_count($bin_str, '1'); // возвращает число вхождений подстроки
...
function summaryRanges(Array $arr) // my
{
	$res = [];
	if ($arr) {
	    $num_start = $num_prev = $arr[0];	
    	$flag = false;
    
    	for ($i = 1; $i < sizeof($arr); $i++) {
    		
    		if (!$flag && $num_prev + 1 == $arr[$i]) {
    			$num_start = $num_prev;
    			$flag = true;
    		} elseif ($flag && $num_prev + 1 != $arr[$i]) {
    			$res[] = "$num_start->$num_prev";
    			$flag = false;
    		} elseif ($flag && $i == sizeof($arr) -1 && $num_prev + 1 == end($arr)) {
    			$res[] = "$num_start->" . end($arr);
    		}
...
function getMirrorMatrix(Array $arrs) // my
{
    $center = sizeof($arrs) / 2;
    $rev_arr = [];
    $count = 1;
    
    foreach ($arrs as $num_arr => $arr) {
        foreach($arr as $k => $v) {
           
            if ($k < $center) {
                $rev_arr[$num_arr][$k] = $v;
                $rev_arr[$num_arr][sizeof($arr) - $count] = $v;
            } elseif ($k == $center) {
               ksort($rev_arr[$num_arr]);
            }
...
function toRna(String $str) // my
{
	$search_arr = ['G' => 'C','C' => 'G','T' => 'A','A' => 'U'];

	$arr = str_split($str);

	foreach ($arr as $key => $value) {
		if (array_key_exists($value, $search_arr)) {
			$arr[$key] = $search_arr[$value];
		} 
	}
...
function toRna(String $nucleotide) // hexlet
{
    $map = [
        'G' => 'C',
        'C' => 'G',
...
function generate(Int $rowNumber) // my
{
	$start = $curr_str = '  1  ';
	...
	for ($i = 1; $i <= $rowNumber; $i++) {
	    ...
	    if ($i < 6) {
	        for ($j = 2; strlen($start) - $j != 3; $j += 2) {
	           $curr_str .= ((int)$start[$j] + (int)$start[$j+2]) . ' '; 
	        }
	    } else {	        
	       for ($j = 2; strlen($start) - $j > 3; $j++) {
	            $pos_del1 = strpos($start, ' ', $j); 
	    	    $pos_del2 = strpos($start, ' ', $pos_del1 + 1);
	    	    
	    	    $curr_str .= ((int)substr($start, $j, $pos_del1 - $j) + (int)substr($start, $pos_del1 + 1, $pos_del2 - $pos_del1 - 1)) . ' ';
	    	    $j = $pos_del1; 
	       }    
	    }     
...
{
	return array_reduce($mails, function ($acc, $mail) {
...
function factorial($num)
{
    $factIter = function ($num, $acc) use (&$factIter) {
        return $num <= 1 ? $acc : $factIter($num - 1, $acc * $num);
    };
...
product(3, 5, $func) // ->
$func(product(3, 4, $func), 5) // ->
$func($func(product(3, 3, $func), 4), 5) // ->
$func($func(3, 4), 5) // -> -6
...
// Рациональный числа как числители со знаменателями:
# makeRat numer denom

// Рациональное число как пары:
# cons car cdr
