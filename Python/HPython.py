...
Задание.
Создайте виртуальное окружение и установите в него пакет flask.
Создайте модуль hello_world.py с содержимом из примера в начале урока.
Разместите рядом Makefile с командой, которая
...
Убедитесь, что текст отображается и изменения строки в исходном файле отражаются на выводимом тексте без ручного перезапуска сервера.
Попробуйте вместо строки вернуть число. В ответ на это браузер должен отобразить ошибку TypeError (и она же должна быть видна в терминале, в котором запущен сервер).
...
Маршрутизация.
Редкий сайт работает так, что пользователи открывают домашнюю страницу и за её пределы не выходят. Гораздо чаще встречается ситуация, когда браузер запрашивает самые разные данные по разным же путям в рамках одного и того же доменного имени.
...
Так уж повелось, что изначально пути в URL означали в первую очередь пути до файлов на диске той машины, которая выступала в роли сервера. Сервер просто отдавал файлы по запросу и даже мог выводить содержимое директории, если вдруг путь указывал на оную. Более того, подобные сайты до сих пор можно встретить: устроены они просто и работают надёжно.

Однако выставлять наружу структуру папок и файлов хочется не всем. Да и менять что-то радикально, после того, как пользователи начнут активно пользоваться сайтом, непросто — у пользователей могут остаться закладки или открытые в браузере вкладки, ссылающиеся на старые версии изменившихся путей!
...
Тот же контент, который получается программным путём — шаблонизированный HTML или генерируемый JSON — отдаёт Web-application. Физических HTML-файлов при этом может и не быть — содержимое страниц создаётся в памяти программы и тут же отдаётся по сети. Так получаются "программные маршруты", которые могу выглядеть вполне "по-файловому" — /blog/posts/2020-01-01.html — но никаким физическим файлам и директориям не соответствуют.
...
def users():
    # …
Каждый путь состоит из сегментов — строк, разделённых слешами (slash, "/"). У статического же пути все сегменты — фиксированные строки.

Функции index и users в примере — обработчики. А каждое применение декоратора route — добавление нового правила маршрутизации. В этом примере правила максимально просты: каждое из них звучит, как "если запрошен такой-то статический адрес, то будет вызван этот обработчик".

Динамические пути.
Статический путь становится динамическим, если в него включают "переменные" (varable sections). Выглядит это так:
...
Упомянутый в коде конвертер int очень прост: он ожидает на входе сегмент, который можно превратить в целое число. А вот конвертер path более интересен: он позволяет захватить (capture) в переменную несколько сегментов за раз, но как минимум один.

path, как говорят, "не жадный" — он захватывает минимально возможное количество сегментов. Это становится важно, когда вы используете комбинации с другими динамическими сегментами. Так в правило /<path:p1>/<path:p2> всегда будет захватывать ровно один сегмент в p1 и все остальные в p2 (минимальный путь, для которого сработает это правило, должен содержать два сегмента).

Помимо path и int ещё доступны конвертеры string (этот используется если не указан никакой другой), float (захватывает числа с плавающей точкой), uuid (захватывает UUID).

Ещё существует конвертер any, подразумевающий "любой из ранее перечисленных" — этот попробует вернуть UUID или число. А если не получится — то один или несколько сегментов в виде строки. Этот конвертер используют редко, так как большинство реальных обработчиков не такое уж и "всеядное".
...
def index():
    return '<h1>Hello</h1>'
Такой обработчик будет работать и при заходе на сам домен foo.com и при явном указании того, что нас интересует главная страница foo.com/index.html.
...
Задание.
В созданном ранее Flask-приложении добавьте обработчик пути /99-bottles, который должен будет отдать HTML-страницу, построенную с помощью шаблонизатора.
Сделайте так, чтобы страница содержала маркированный список <ul> c элементами <li>. Элементы должны содержать строки "99 бутылок чего-то стояло на столе, одна упала и разбилась.", "98 бутылок..." и так далее вплоть до "Нет больше бутылок на столе.". Вам пригодится цикл for в его шаблонной версии.
...
Задание.
Добавьте в ваше приложение маршрут /args.
Реализуйте обработчик для этого маршрута, возвращающий HTML-страницу c таблицей.
Выводите в таблицу имена и значения параметров из query string. Помните о том, что каждый параметр может иметь несколько значений.
...
def foo():
    ...
    return response
Задание
Добавьте обработчик в ваше приложение (путь выберите сами).
Сделайте так, чтобы обработчик возвращал параметры запроса в виде JSON-объекта. Добейтесь того, чтобы ключи соответствовали именам параметров, а значения — спискам значений для каждого параметра (помним, в запросе параметр может встречаться несколько раз!).
