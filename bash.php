...
ls -l projects/	// просмотр директории. Нажать tab для автокомплита названия дирекории. Опция -a, показать все файлы, включая скрытые (имена которых начинаются с точки). Опция '-l' - более подробная информация о файлах(права на файл, имя владельца, время последнего изменения файла и его размер). Опция -d' - выводить только директории. '-R' - выводит информацию о файлах находящихся в директории рекурсивно. Опция 'i' может использоваться для отображения числа инодов для объектов в списке файловой системы =>
/*
...
ls -dl /usr/local /* увидеть общее количество ссылок ведущих на 5120 инод =>
...
// Выполнение find может занять некоторое время, т.к. ей необходимо прочитать каждую директорию в которой выполняется поиск. Оказывается, что команда locate может ускорить процесс использую внешнюю базу данных, генерируемую updatedb (updatedb мы рассмотрим ниже).
