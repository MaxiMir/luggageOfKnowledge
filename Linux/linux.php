...
id // группы пользователей => uid=1000(vagrant) gid=999(team) groups=999(team), 998(docker) ; uid - уникальный индетификатор


>>>>> Суперпользователь <<<<<<<

id // => 
uid=0(root) gid=0(root) groups=0(root) // root - суперпользователь, имеет неограниченный доступ. Домашняя директория суперпользователя: /root
