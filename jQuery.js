...
$j = Query.noConflict();// удалить псевдоним '$' из глобального пространства имен. Чтобы код с $ необходимо его передать вначале: jQuery(function($) {...}); 
...
jQuery(function($){
	function func() {
		...
		if (img.css('display') !== 'none'){
			...
			$('#mytext').hide(500, 'linear', function()т{ // динамика выполнения анимаций: 'linear' - равномерно, 'swing' - по умолч. с небольшим ускорением. Есть и др. 
				img.show(1000); 
				$(this).remove();
			});	
	}
...
.prevAll() // находит элементы, все которые лежат до каждого из выбранных 
