...
jQuery.noConflict();// удалить псевдоним '$' из глобального пространства имен. Чтобы код с $ необходимо его передать вначале: jQuery(function($) {...}); 

jQuery('.main_items'); // селектор по классу
jQuery('#head'); // селектор по id
jQuery('p', '.nav').html(); // 2-й параметр селектор родителя
jQuery('p').html(); // селектор по тегу, html() - метод возвращает содержимое тега первого эл-та выборки
...
jQuery('p', '.nav').append(h5); // append() -  добавляет контент внутрь выбранных элементов в конец, после имеющихся
...
'parent + next' // элемент next, который следует непосредственно за элементом prev (один уровень вложенности)

'parent ~ next' // элементы next, которые следуют за элементами из prev (один уровень вложенности)

var obj = $('div.grid_12 ~ div');
alert(obj[0]); // Объект JS - 1 элемент выборки  => HTMLDivElement
for(v in obj){
	document.write('string' + v + ' - <strong>' + obj[v] + '<br>'); // в свойствах объекта есть методы JQuery
}
...
$'[first][second][..'; // элементы, соответсвующие всем заданным  условиям на атрибуты одновременно
$'[class|=full], [name], [data-type="submit hello"]' // если 1 слово ковычки можно не ставить, т.е data-type=submit
...
img.toggleClass(function(ind,cl){ // работа с коллекцией элементов
	if(cl == 'redClass'){
		return 'greenClass';
	}else if(cl == 'redClass greenClass'){ // функция удалит оба класса
...
$('input').val(function(ind,v){ // => в атрибут value будет записан порядковый номер элемента 
	return ind;
}); 
...
function func(){
	...
	if(img.attr('src') == 'images/1.jpg'){ // => возвращает содержимое атрибута 'src'
		...
		img.attr('src', function(ind,v){ // in - порядковый номер эл-та в выборке. Код функции будет выполнен для каждого эл-та в отбельности
...
$(document).scrollTop(1000); // текущая html страница. Перемещает горизонтальную полосу прокрутки
...
.after() // добавляет заданное содержимое после элементов на странице
.insertAfter()
$( selector ).after( content )
$( content ).insertAfter( selector )

.before() // добавляет заданное содержимое перед элементами на странице
.insertBefore()
...
jQuery(function($){
	...
	var html = $('section_main').html(function(ind,oldVal){ // ind - индекс эл-та, oldVal - тек.содержимое элемента
		if(ind%2 == 0){
	...
	form.find('span').text('Field - ' + (count + 1)); //  find() - поиск потомков внутри каждого элемента в текущем наборе соотвествующих элементов с фильтрацией по селектору, объекту jQuery или элементу.
...
.fadeIn() // скрывает/показывает элементы на странице
...
function func(){
	...
	if(img.css('display') !== 'none'){
		...
		img.hide('fast', function(){ // ф-я выполняется после заврешения анимац. действий
			...
			text.insertAfter($(this)).hide().show(1000); // в this - текущий элемент img	
		});
	}else{
	...
	$('grid_12 h2') // Все пользовательские очереди необходимо запускат в вручную
	...
	$('grid_12 h2').dequeue('myque') // Все пользовательские очереди необходимо запускат в вручную
}
...
.mouseleave() // устанавливает обработчик выхода курсора из области элемента, либо, запускает это событие. Появление этого события, отработано лучше, чем стандратного mouseout. Не имеет обладает св-твом `всплытия`

.mousemove() // устанавливает обработчик движения крусора в области элемента, либо, запускает это событие
...
jQuery(window).unload(function(){}); // метод - не кроссбраузерный
...
<div id="scroll" style="left: 90%; top: 90%; opacity: 0.6; width: 100px; height: 30px; background-color: gray; display: none">UP

jQuery(function($) {
	jQuery(window).scroll(function(){ // функция поднимает скролл вверх при клике на появл. элемент
		if($(this).scrollTop() > 500){ // на сколько px передвинули скролл
...
$('.sf-menu li').on({click:function(){}, resize: function(){}}); // событие:функция-обработчик

--

$('.sf-menu li').click(function(e){ // обработчик нескольких событий, указываются через ' '
...
$('.sf-menu').off(); // удаляем ранее зарегистрированный обработчик
...
.nextAll() // находит элементы, все которые лежат после каждого из выбранных элементов 
...
.sublings() // находит все соседние элементы (под соседними понимаются элементы с общим родителем)
...
jQuery(function($){
	...
	$('li').(-1);filter(function(ind,val){ // код функции будет вызываться для каждого эл-та выборки
...
.each() // вызывает заданную функция для каждого элемента набора
...
result.get(); // конвертация из объекта в массив
...
.andSelf().css("border","1px solid red"); // добавляет предыдущий набор jQuery к текущему в пределах одной цепочки команд и возвращает объединенный набор. Теперь объединим текущий объект jQuery с предыдущим и установим красную границу вокруг всех элементов <div> и элементов <span> внутри них.

$('grid_7').children().each(function(){ // текст в div .grid_7 метод children() проигнорирует
	alert($(this).text());
});
// а здесь нет:
$('grid_7').contents().each(function(){}); // возвращает все дочерние элементы выбранных элементов, а также текстовое содержимое
...
$('.btn').click(function(e){
	...
	$('#ajax').html('<span>Отправка</span>').fadeIn(1000,function(){ // #ajax - пустой блок div
		...
		*/	
		...
		$.param(result); // преобразует объект/массив/массив объектов в строку, которая соответвует формату передачи данных через URL*/
		...
		$.ajaxSetup{ // метод для задания будущих параметров запроса для отправки на сервер
			...
			beforeSend: function(jqXHR){ // функция будет вызвана перед отправкой AJAX запроса на сервер
				// применяется как правило для модификации заголовков перед отправкой
			},
...
jQuery(function($){ 
	...
	JSON.stringify(result); // приведение в строку формата JSON => {'one':100, 'two':20,'three':{'val1':'Ben','val2':'Bob','val3':'!'},'four':'string'}	
	
	--
	
	var newVar = 50; // локальная переменная
	console.log(newVar); // => 50
	$.globalEval('newVar="Hello world!;alert(newVar);"'); // а здесь глобальная => Hello world!
	console.log(newVar); // => 50
	// выполняет скрипт в глобальной области видимости
	...
	var html = $.parseHTML(str); // преобразует переданную строку с HTML-кодом в массив DOM-узлов самого верхнего уровня. Пробелы в начале и конце строки тоже будут преобразованы в текстовые узлы. Чтобы этого избежать можно воспользоваться функцией jQuery.trim().
	$('.sf-menu').append(html);
