...
################ СТРОГАЯ ТИПИЗАЦИЯ ################
function sum(int ...$nums): int {} // $nums и возвращаемое ф-ей значение приводятся к типу int. Результат приводится к int
...
*/
	 ...
	 */
	 ...
	 $serialized = serialize($obj); // перед запуском запустит __sleep() и создаст указанные свойства
	 echo "{$serialized}\n";
	 ...
	 */
	 ...
	 $storage[$object1] = 'Jorn'; // объект в качестве ключа
	 ...
	 */
	 ...
	 // Обзор функции:
	 ReflectionFunction::export(new ReflectionFunction('sayHello'));
	 ...
	 */
	 ...
	 function curlHeaderCallback($curl, $headers) // переопределение заголовков
	 {
		 ...
		 header('Content-Disposition: attachment; filename="file-name.zip"'); // ожидаемый контент ответа будет отображаться в браузере, как вэб-страница или часть вэб-страницы, или же как вложение, которое затем может быть скачано и сохранено локально.
	 ...
	 */
	 ...
	 $pattern_in_quot = "/^4\\*\\*$"; // в двойных ковычках (т.к в них '\' считается экранирующим )
	 ...
	 */
	 ...
	 Стиль построения архитектуры распределенного приложения
		 ...
		 Сетевой протокол должен поддерживать кэширование, не должен сохранять информацию о состоянии между парами "запрос-ответ".
