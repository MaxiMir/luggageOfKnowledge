	 ...
	 class TestForm extends Model
	 {
		  ...
		  public function rules()
		  {
				return [
					...
					['name', 'myRule'],
					 // собственный валидатор, должен иметь уник. название
					['text', 'trim'],
					 // после потери фокуса значение поля пропускают через trim
					['text', 'safe']
					 // валидатор данные будут доступны без проверки. Лучше использовать trim
				];
		  }
    ...
    if ($model->load(Yii::$app->request->post())) {  // если данные POST успешно загружены
		  if (!$model->validate()) { // и данные формы не валидны
				Yii::$app->session->setFlash('error',
					'Ошибка'); // flash сообщения (данные после их запроса будут удалены из сессии)
		  } else {
    ...
    public function actionShow()
	 {
		  ...
		  // count() - количество выбраннных записей.F
		  ...
		  $cats = Category::findBySql($query,
			  [':search' => '%pp%'])->all(); // findBySql - метод для выполнения SQL запроса
