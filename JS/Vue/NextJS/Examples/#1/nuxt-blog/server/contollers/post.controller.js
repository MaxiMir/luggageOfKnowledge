...
module.exports.create = async (req, res) => {
  const post = new Post({
    ...
    text: req.body.text,
    imageUrl: `/${req.file.filename}` // путь до картинки после обработки multer
  })
...
module.exports.getById = async (req, res) => {
  try {
    const post = await (await Post.findById(req.params.id)).populate('comments').execPopulate((error, post) => { // + получаем список комментариев
      res.json(post)
    })
...
module.exports.update = async (req, res) => {
  ...
  try {
    const post = await Post.findOneAndUpdate({
      _id: req.params.id,
    }, {$set}, {new: true}) // {new: true} - вернуть обновленный объект
