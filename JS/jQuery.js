...
let jqXHR = $.get('server.php', result, function (data, status, jqXHR) {
...
jqXHR.done(function () {
}); // функция-обработчик выполняется в случае успешного выполнения запроса к серверу
jqXHR.fail(function () {
}); // функция-обработчик выполняется в случае ошибочного выполнения запроса к серверу
jqXHR.always(function () {
}); // функция-обработчик выполняется в любом случае
jqXHR.then(function () {
}, function (data) {
}); // 1 функция-обработчик выполняется в случае успешного выполнения запроса к серверу, 2 - в случае ошибочного

jqXHR = $.post('server.php', result, function (data, status, jqXHR) { }, 'json'); // запрос методом POST

jqXHR = $.getJSON('server.php', result, function (data, status, jqXHR) { }) // запрос JSON данных у сервера методом GET
// <->
$.ajax({
  url: 'server.php',
  dataType: 'json',
...
let result = $.extend(object1, object2); // метод объединяет содержимое 2-х или более объектов. Возвращает 1-й исходный объект, при результат операции записывается в 1-й переданный объект
...
const newVar = 50; // локальная переменная
...
const newArr = $.grap(arr, func); // ищет в заданном массиве элементы, удовлетворяющие условиям фильтрующей функции. Возвращает массив с найденными элементами (в исходный массив изменения не вносятся). => [1,2,5,6,7,8,9,10];
...
$.map( obj, function( el, ind ) { // выполняет заданную функцию для каждого элемента массива или каждого поля объекта в отдельности. Значения, полученные в результате выполнения этой функции помещаются в новый массив и возвращаются как результат работы функции.
