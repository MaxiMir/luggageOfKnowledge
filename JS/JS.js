...
document.querySelector('h2').onclick = function () {
    ...
    document.querySelector('.wrap').insertBefore('newDiv', document.querySelector('.wrap' +
        'item')); // 1-й аргумент - вставляемый узел, 2-й элемент - дочерний узел, перед которым необходимо добавить новый узел. с null во втором аргументе <-> appendChild()
...
document.getElementById('firstForm').addEventListener('click', function (e) { // обработчик для всей формы
    ...
    ; // e.target - содержит элемент, который является целью события
    console.log(this);
}, false);
...
window.onload = function (e) {
	...
	function hendler(e) { // e - объект возникшего события
		...
		request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); // заголовок при отправке информации на сервер
