	...
	_garderoboAssistantWidget.init = async (widgetSettings) => {
		...
		let settings = {
			...
			bgImages: {
				...
				nextPageDisabled: "data:image/png;base64,",
		...
		const getQuestionData = () => {
			...
			const html = `
					...
					<div class="ai-wgt__answers fl-column-center">
						<div class="ai-wgt__answer ai-wgt__link mb-15" data-answer-id="4">Соглашусь</div>
						<div class="ai-wgt__answer ai-wgt__link mb-15" data-answer-id="1">Не про меня</div>
			...
			const initElementHandler = (element, cb) => {
			...
			const answerBtnHandler = e => {
				...
				const {answerId} = currAnswer.dataset;
				const nextPageBtn = container.querySelector(".ai-wgt__next-page");
				const {action} = nextPageBtn.dataset;
				
				nextPageBtn.dataset.action = answerId;
				
				if (action) {
					const prevAnswer = container.querySelector(`[data-answer-id='${action}']`);
					prevAnswer.classList.remove("active");
				}
				
				currAnswer.classList.add("active");
				initElementHandler(nextPageBtn, nextPageBtnHandler);
			};
			
			const nextPageBtnHandler = async e => {
				const nextPageBtn = e.currentTarget;
				const {action} = nextPageBtn.dataset;
